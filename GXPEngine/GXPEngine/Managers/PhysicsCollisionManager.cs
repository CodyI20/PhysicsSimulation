...
namespace GXPEngine.Managers
{
	public class PhysicsCollisionManager
	{
		...
		{
			float distance = Vec2.Displacement(a.position, b.position).Length();
			float radiusA = new Vec2(a.width, a.height).Length()/2;
			float radiusB = new Vec2(b.width, b.height).Length()/2;
